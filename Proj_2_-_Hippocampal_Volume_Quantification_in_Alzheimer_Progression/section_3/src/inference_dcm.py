...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    txt += f"Study Description: {header.StudyDescription}\n"
    txt += f"Series Description: {header.SeriesDescription}\n"
    txt += f"Modality: {header.Modality}\n"
    txt += f"Image Type: {header.ImageType}\n"
    txt += f"Hippocampal total volume: {inference['total']}\n"
    txt += f"Anterior volume: {inference['anterior']}\n"
    txt += f"Posterior volume: {inference['posterior']}\n"
    ...
    org_slc = orig_vol[18, :, :]
    org_nd_img = np.flip((org_slc/np.max(org_slc))*255).T.astype(np.uint8)
    ...
    pred_slc = pred_vol[18, :, :]
    pred_nd_img = np.flip((pred_slc/np.max(pred_slc))*255).T.astype(np.uint8)
    # This is how you create a PIL image from numpy array
    pred_pil_i = Image.fromarray(pred_nd_img, mode="L").convert("RGBA").resize((250, 250))
    # Paste the PIL image into our main report image object (pimg)
    pimg.paste(pred_pil_i, box=(350, 500))
...
def get_series_for_inference(path):
    ...
    dicoms = [pydicom.dcmread(p) for p in glob.glob(f'{path}/*HCropVolume/*.dcm')]
...
if __name__ == "__main__":
    ...
    inference_agent = UNetInferenceAgent( device="cpu", parameter_file_path= r"./model/model.pth")
